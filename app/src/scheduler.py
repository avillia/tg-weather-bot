...
async def send_forecast_for_user(bot: Bot, user_id: int, latitude: float, longitude: float):
...
def schedule_forecast(bot: Bot, forecast_time: Union[time, datetime], user: User):
    ...
    scheduled_time = DateTrigger(parse_time(str(forecast_time)), timezone(user.timezone))
    scheduler.add_job(send_forecast_for_user,
                      args=[bot, user.id, user.last_latitude, user.last_longitude],
                      trigger=scheduled_time, replace_existing=True)
...
def setup_scheduler(bot: Bot):
    scheduler.add_job(schedule_forecasts_in_db, args=[bot], replace_existing=True, trigger="cron", hour=0, minute=0)
    scheduler.start()
